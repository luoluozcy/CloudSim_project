...
package org.cloudbus.cloudsim.brite.Graph;
...
public class Graph /*TODO: implements graphAlgs*/ {
	 ...
	 */
	public boolean hasNode(Integer id) { return Nodes.containsKey(id); }
	public boolean hasNode(int nID) {return Nodes.containsKey(new Integer(nID)); }

	/** 
	not implemented yet
	 */
	public void removeNode(Node n) {
		//TODO:  remove node, remove incident edges, remove resulting disconnected nodes
	}
	...
	/**
       Given a node, returns all node that are incident to this
	...
	public Node[] getNodesArray() { return (Node[]) Nodes.values().toArray(new Node[numNodes]); }
	...
	public Edge[] getEdgesArray() { return  (Edge[]) Edges.values().toArray(new Edge[Edges.size()]); }
	...
	public boolean isConnected() {
		...
		for (int i=0; i<nodesV.length; ++i) {
			...
			if (n.getColor() != GraphConstants.COLOR_BLACK){
				Util.DEBUG("***** Node " + n.getID() //+ " of AS: "+ ((RouterNodeConf)n.getNodeConf()).getCorrAS()
						+" is not black");
	...
	//Returns the Edge connecting src and dst
	public Edge getEdge(Node src, Node dst) {
		//hasEdge utiliza o Edge.computeID para dizer se 
		//ha aresta entre src e dst
		int intID = Edge.computeID(src.getID(), dst.getID());
		if (intID==-1) {
			long longID = Edge.computeLongID(src.getID(), dst.getID());
			//Jah tenho o id unico
			if (Edges.containsKey(new Long(longID)))
				//Forneco a chave e me retorna o objeto
				// an Edge has been found that connects src and dst
				return (Edge) Edges.get(longID);
		} else {
			if (Edges.containsKey(new Integer(intID)))
				//Forneco a chave e me retorna o objeto
				return (Edge) Edges.get(intID);
		}//fim else
		// no Edge was found that connects src and dst
		return null;
	}//fim getEdge
